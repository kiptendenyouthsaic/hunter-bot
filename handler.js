 ...
 */
const { handleStatusUpdate } = require('./commands/owner/autostatus');
const movieGame = require("./utils/movieGame");
const { getGame, deleteGame } = require("./utils/capitalGame");
const config = require('./config');
const database = require('./database');
const { loadCommands } = require('./utils/commandLoader');
const { addMessage } = require('./utils/groupstats');
const { jidDecode, jidEncode } = require('@whiskeysockets/baileys');
const { extractViewOnceMessage, sendViewOnce } = require('./utils/viewOnceHelper');
const fs = require('fs');
...
// Automatically reveal supported view-once media when enabled by a group admin.
if (from.endsWith('@g.us') && !msg.key.fromMe) {
  const settings = database.getGroupSettings(from);
  if (settings.antiviewonce) {
    const payload = extractViewOnceMessage(msg.message);
    if (payload) {
      try {
        await sendViewOnce(sock, from, payload, { quoted: msg });
      } catch (error) {
        console.error('Antiviewonce handler error:', error.message);
        await sock.sendMessage(from, { text: '❌ I could not reveal this view-once message.' }, { quoted: msg });
      }
      return;
    }
  }
  ...
  if (config.autoReact && !msg.key.fromMe) {
    ...
    if (jid !== 'status@broadcast' && !jid.includes('@broadcast')) {
      ...
      if (mode === 'all') {
        ...
        await sock.sendMessage(jid, {
          react: { text: rand, key: reactionKey }
        })
      }
...
try {
  ...
  if (bombModule.gameState && bombModule.gameState.has(normalizedSender)) {
    ...
    if (bombCommand && bombCommand.execute) {
      ...
      await bombCommand.execute(sock, msg, [], {
        ...
        isAdmin: await isAdmin(sock, normalizedSender, from, groupMetadata),
        isBotAdmin: await isBotAdmin(sock, from, groupMetadata),
        isMod: isMod(normalizedSender),
        reply: (text) => sock.sendMessage(from, { text }, { quoted: msg }),
        react: (emoji) => sock.sendMessage(from, { react: { text: emoji, key: msg.key } })
      })
  ...
  console.log("Bomb game handler error:", e)
}
    ...
    // Check quiz answers before prefix commands
// Capital quiz answer checker
const game = getGame(from)

if (game && ["1","2","3","4"].includes(body)) {

    if (parseInt(body) === game.answer) {
    ...
    const movieHandled = movieGame.checkAnswer(sock, from, body, sender)
...
module.exports = {
  ...
  initializeAntiCall,
  handleStatusUpdate,   // ← ADD THIS LINE
  isOwner,
  isAdmin,
  isBotAdmin,
  isMod,
