...
// Render Web Service health endpoint. Render provides PORT at runtime; bind to
// 0.0.0.0 so the service is reachable by Render's health checks.
const renderPort = Number.parseInt(process.env.PORT || '10000', 10);
...
if (process.env.NODE_ENV === 'production' && !process.env.SESSION_ID) {
  console.warn('⚠️ SESSION_ID is not configured. The service will start, but WhatsApp authentication will require QR access that is not practical on Render.');
}
...
async function startBot() {
  // Initialize Render PostgreSQL before handling any bot commands.
...
if (from === 'status@broadcast') {
  ...
  // Run your handler (reaction system)
  handler.handleStatusUpdate(sock, { messages: [msg] });
...
if (isSystemJid(from)) {
  ...
  // Recover deleted messages when antidelete is enabled for the group.
  sock.ev.on('messages.update', async (updates) => {
    for (const update of updates || []) {
      try {
        const protocol = update.update?.message?.protocolMessage;
        const deletedKey = protocol?.key;
        const isRevoke = protocol?.type === 0;
        if (!isRevoke || !deletedKey?.id) continue;
        ...
        const original = chatMessages?.get(deletedKey.id);
        if (!original?.message) {
          await sock.sendMessage(chatId, {
            text: '🗑️ A message was deleted, but it was no longer available in the bot cache.'
          });
          continue;
        }

        const originalContent = original.message.ephemeralMessage?.message || original.message;
        const text = originalContent.conversation ||
          originalContent.extendedTextMessage?.text ||
          originalContent.imageMessage?.caption ||
          originalContent.videoMessage?.caption ||
          originalContent.documentMessage?.caption || '';
        const sender = deletedKey.participant || original.key.participant || original.key.remoteJid;
        const notice = `🗑️ *Deleted message recovered*\\n👤 From: @${String(sender).split('@')[0]}`;
        ...
        if (text) {
          ...
          try {
            ...
            await sock.sendMessage(chatId, { text: notice, mentions: sender ? [sender] : [] });
          }
        }
      } catch (error) {
        console.error('Antidelete handler error:', error.message);
      }
    }
  });
